<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header">
      <h5 class="modal-title" id="staticBackdropLabel">
        Loans # <?php echo $loan->id ?>
        <br>
        Customer: <?= $loan->customer_name; ?>
        </h5>
      <div class="d-flex flex-row-reverse">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fas fa-times fa-sm"></i></button>
        <button type="button" class="close" onclick="window.print();"><i class="fas fa-print fa-sm"></i></button>
      </div>
    </div>
    <div class="modal-body">
      <div class="row">
        <div class="col-md-12">
          <div class="table-responsive">
            <div class="clearfix mb-2">
              <div class="float-left">
                Credit Amount: <?= $loan->credit_amount; ?>
                <br>
                Interest Credit: <?= $loan->interest_amount.'%'; ?>
                <br>
                Number of installments: <?= $loan->num_fee; ?>
                <br>
                Fee amount: <?= $loan->fee_amount; ?>
                <br>
                currency type: <?= strtoupper($loan->short_name); ?>
                <br>
              </div>
              <div class="float-right">
                 Credit Date: <?= $loan->date; ?>
                 <br>
                 Payment method: <?= $loan->payment_m; ?>
                 <br>
                 Loan Status: <?= $loan->status ? 'Pending': 'Paid'; ?>
               </div>
            </div>

            <div class="table-responsive">
              <table class="table table-striped table-condensed">
                <thead>
                   <tr class="active">
                     <th>Fee Number</th>
                     <th class="col-xs-2">Payment Date</th>
                     <th class="col-xs-2 text-right">Total pay</th>
                     <th class="col-xs-2 text-center">Status</th>
                   </tr>
                </thead>
                <tbody>
                  <?php
                  if ($items) {
                    $i = 0;
                    foreach ($items as $item) {
                      echo '<tr>';
                      echo '<td>'.++$i.'</td>';
                      echo '<td>'.$item->date.'</td>';
                      echo '<td class="text-right">'.$item->fee_amount.'</td>';
                      $status = ($item->status) ? 'Pendiente' : 'Cancelado' ;
                      echo '<td class="text-center">'.$status.'</td>';
                      echo '</tr>';
                    }
                  }
                  ?>
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
